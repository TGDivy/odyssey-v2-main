...
| §20 Apple ecosystem | `partial` | iOS/Watch/macOS shells, widgets/intents/share targets, portable data/auth/sync packages, guarded local AVFoundation voice capture, selected-only PhotosUI capture, system file import, permission-gated Odyssey-owned food HealthKit writes, protected atomic extension commands, offline/private App Intents, generic widget/Control Center Capture/Food routing, and bounded crash-idempotent iPhone drains | Wire Watch transport; prove App Intents/WidgetKit/controls/HealthKit on device; add incremental health reads, EventKit/location and validation. |
...
| 1.2 capture/personal library | `partial` | Offline durable text/media capture and review, atomic food-preset lifecycle/ranking and occurrence record/correct/void, registered food events, ranked iPhone quick log, portable-tested HealthKit reconciliation, protected extension handoff, shortcuts, generic widget/control sheet routing, and crash-idempotent iPhone drains exist | Wire Watch transport; add playback, provider interpretation and event consumption; prove App Intents/WidgetKit/HealthKit/accessibility/warm timing on device. |
...
- The complete portable Swift package reports 128 tests passing under the
