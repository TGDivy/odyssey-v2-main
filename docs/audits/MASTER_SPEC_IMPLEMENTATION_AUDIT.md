...
| §18 experience architecture | `partial` | bounded iPhone Now with six deterministic states, explicit empty/intentional-silence handling, accepted Season thread, next transition, generated one-screen Tomorrow Map, reason-coded correction, rendered guilt-free re-entry, explicit text/voice/photo/file capture, local-first ranked food quick-log/create/correct/void source with payload-free monotonic warm-path instrumentation and ephemeral owner result, forensic capture-detail Archive, typed reviewed life-model Workshop, accepted Season landscape/plain-language Map, and sync/repair surfaces | Decision/evidence flows, physical warm-path proof, Xcode accessibility and complete platform depth. |
...
| §20 Apple ecosystem | `partial` | iOS/Watch/macOS shells, widgets/intents/share targets, portable data/auth/sync packages, guarded local AVFoundation voice capture, selected-only PhotosUI capture, system file import, permission-gated Odyssey-owned food HealthKit writes, anchored local-only HealthKit reads with guarded scoped observer delivery, a full-access bounded EventKit mirror, guarded foreground Core Location-to-attributed WeatherKit handoff with coordinate-free local cache/diagnostics, an eight-capability runtime matrix, protected extension commands, App Intents, privacy-sensitive freshness-aware Now widget cache/rendering, widget/Control Center routing, crash-idempotent iPhone drains, and a protected receipt-bound WatchConnectivity text/food outbox | Prove Apple paths, accessibility, App Group/Data Protection behavior, widget scheduling/redaction, suspension/relaunch behavior, and performance on signed devices. |
| §21 integrations | `partial` | local Health/EventKit/WeatherKit/Core Location adapters, synthetic fixtures, durable revocable mirrors, independent failure preservation, broad connector-health diagnostics, and durable prior-opt-in activation; external provider credentials remain disabled | Finish contacts/photos semantic ingestion and add supported OAuth/webhook adapters only after value/privacy review. |
| §22 data architecture | `partial` | append-only ledger, projections, provenance contracts, streaming SHA-256, schema-v5 hash-verified bounded local application state, bounded protected provider-import buffering, protected opaque local attachment manifests plus tested filesystem/ledger handoff and reconciliation, encrypted owner export/import foundations, accepted life-model history and 17 backend migrations | Attachment tombstone/retention lifecycle, complete semantic services, selective memory, redaction and scale budgets. |
...
| §38 roadmap | `partial` | Edition 0 substrate and Milestones 1.1–1.4 repository slices exist in dependency order | Milestone 1.4 Apple proof, Milestone 1.5, later Edition 1 gates, and Editions 2–4 remain. |
...
| 1.3 Apple context adapters | `implemented` | HealthKit anchored import and guarded scoped observation, bounded full-access EventKit reconciliation, guarded one-shot foreground Location-to-Weather handoff, complete eight-capability and four-connector health UI, revocable coordinate-free local mirrors, explicit degraded behavior, durable prior-opt-in activation, synthetic denial/provider/travel/local-day fixtures, source metadata, duplicate/conflict/deletion/cancellation handling, and attribution satisfy the repository slice | Xcode compilation, signing, accessibility, live services, OS scheduling, and physical-device permission/delivery behavior remain owner-only proof. |
| 1.4 Now/Tomorrow Map v1 | `implemented` | Six-state deterministic native projection, explicit empty versus intentional silence, accepted Season thread and next transition, named-zone one-screen Tomorrow Map, 24-hour reason-coded correction, schema-v5 durable visit/correction state, three-day deterministic re-entry, bounded privacy-sensitive App Group widget snapshot, real widget freshness states, lifecycle refresh composition, and focused portable tests exist; Xcode/accessibility/App Group/timeline/device proof remains owner-only. |
...
| Now can intentionally show silence | `repo` | `implemented` | Native projection and UI distinguish fresh-context intentional Clear, owner-requested quiet, incomplete empty context, known Open, and unavailable projection; portable policy tests and parser checks pass, while Xcode/accessibility proof remains owner-only. |
| No universal Life Score or people ranking | `repo` | `verified` | Constitution and implementation omit both. |
| Guilt-free re-entry | `repo` | `implemented` | Durable last-seen state, three-day gate, deterministic source changes, at most three summaries, one generic question, stale expiry, Continue/Revise Season/Stay Quiet, backlog suppression, no absence penalty, native rendering, and focused tests exist; final SwiftUI proof is owner-only. |
...
| Incremental/revocable local context mirrors | `owner` | `partial` | Health uses per-kind anchors and scoped observation, calendar uses a bounded full snapshot, Location and Weather each use one independently revocable mutable broad-place record. They preserve appropriate prior context on provider/permission failure, require durable prior activation for automatic resume, revoke local records/markers without source mutation, and avoid the sync outbox. Device proof remains. |
| Permission denial degrades gracefully | `owner` | `partial` | Microphone, Health read/write, calendar full-read, foreground Location, and WeatherKit failure paths preserve local capture/logging and appropriate existing mirrors; Location denial suppresses Weather. Xcode/device proof remains. |
| Cached freshness-aware widget | `owner` | `implemented` | The app atomically publishes a bounded privacy-sensitive App Group snapshot, and the real widget distinguishes current/stale/unavailable/preview states without titles, payloads, values, coordinates, or exact refresh promises; Xcode entitlement, lock-screen redaction, timeline, and device proof remain. |
...
- The complete portable Swift package reports 200 tests passing under the
  ...
  scoped revocable Health change observation and durable activation gates,
  mutable calendar update/cancellation/window reconciliation and timezone rules,
  one-shot broad Location resolution, denied handoff suppression, named-zone
  travel/local-day reassignment, and background acquisition suppression,
  coordinate-free mutable Weather snapshots, attribution/freshness validation,
  provider failure preservation and local revocation, deterministic six-state
  native context composition, empty-versus-intentional silence, named-zone
  Tomorrow Map pressure/open-period rules, bounded reason-coded corrections,
  schema-v5 hash-verified local application state, deterministic three-day
  re-entry, and bounded atomic widget snapshots,
...
- iOS, widget/control, App Intent and Watch sources, including the rendered Now,
  Tomorrow Map, re-entry/correction flow, real freshness-aware private widget,
  guarded voice,
  selected-only photo/file capture, ranked food flows, HealthKit reads/writes,
  HealthKit observer delivery, EventKit, Core Location and WeatherKit adapters,
  and WatchConnectivity adapters, passed Swift parser/structure validation;
  guarded HealthKit, Core Location, and WeatherKit code also passed targeted
  strict-concurrency checks against disposable modules shaped from public
  signatures. `apple/project.yml` passed YAML structure checks. SwiftUI,
  WidgetKit, AppIntents, WatchConnectivity, PhotosUI, AVFoundation, HealthKit,
  EventKit, Core Location, and WeatherKit still require Xcode type-checking.
