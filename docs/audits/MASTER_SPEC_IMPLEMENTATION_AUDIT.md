...
| 1.2 capture/personal library | `partial` | Offline durable text/media capture and review, atomic food-preset lifecycle/ranking and occurrence record/correct/void, registered food events, ranked iPhone quick log, portable-tested HealthKit reconciliation, protected extension handoff, offline text shortcut, private food-open shortcut, and crash-idempotent iPhone drains exist | Wire widget/control/Watch producers; add playback, provider interpretation and event consumption; prove App Intents/HealthKit/accessibility/warm timing on device. |
...
- The complete portable Swift package reports 126 tests passing under the
