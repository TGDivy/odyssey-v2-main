...
| §12 memory architecture | `partial` | immutable ledger/projections, capture/archive contracts, source-linked interpretation/review, protected opaque local-only attachment objects with staged recovery, rebuild/export tools and immutable context snapshots | Media/capture composition, normalized admission, retrieval plans, contradiction, condensation, forgetting/redaction and broader source annotation services. |
...
| §22 data architecture | `partial` | append-only ledger, projections, provenance contracts, streaming SHA-256, protected opaque local attachment manifests/reconciliation, encrypted owner export/import foundations, accepted life-model history and 17 migrations | Media composition, attachment tombstone/retention lifecycle, complete semantic services, selective memory, redaction and scale budgets. |
...
| 1.2 capture/personal library | `partial` | Offline durable text capture, local source-inspectable Archive review, protected local-only attachment staging/integrity/recovery, source-linked interpretation versions, coalesced async execution, atomic derivative/outbox commit and optimistic append-only owner review exist; media composition/UI, provider interpretation, food/quick logging and HealthKit write remain. |
...
- The complete portable Swift package reports 93 tests passing under the
  ...
  source-linked capture interpretation, protected local attachment storage,
